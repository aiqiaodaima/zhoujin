<template>
    <table v-if="treeData.enterpriseName">
      <tr>
        ...
        >
          <div :class="{node: true, hasMate: treeData.mate}">
            ...
            >
             ...
             <a> <div class="name">{{treeData.enterpriseName}}</div></a>
            </div>
            <template v-if="Array.isArray(treeData.mate) && treeData.mate.length">
              <div class="person" v-for="(mate, mateIndex) in treeData.mate" :key="treeData.enterpriseName+mateIndex"
...
.extend_handle{position: absolute;left:50%;bottom:30px; width:10px;height: 10px;transform: translate3d(-5px,0,0);cursor: pointer;}
...
.node .person{position: relative; display: inline-block;z-index: 2;width:15em; overflow: hidden;}
