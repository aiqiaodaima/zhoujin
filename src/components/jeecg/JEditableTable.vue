    ...
    <div :id="`${caseId}inputTable`" class="input-table">
      ...
      <div class="scroll-view" ref="scrollView" :style="{'max-height':maxHeight+'px'}">
        ...
        <div :id="`${caseId}tbody`" class="tbody" :style="tbodyStyle">
              ...
              "
                ...
                <div
                  ...
                  <template v-for="(id,i) in [`${col.key}${row.id}`]">
                    ...
                    <!-- else (normal) -->
                    <template v-else>
                      <span v-if="typeof col.customRender === 'function'" :key="i" v-bind="buildProps(row,col)">{{ col.customRender(inputValues[rowIndex][col.key]) }}</span>
          ...
          <div v-if="actionButton" class="action-button">
            <a-button style="width: 100%; margin-top: 16px; margin-bottom: 8px" type="dashed" icon="plus" @click="handleClickAdd">新增</a-button>
          </div>
        </div>
      </div>
    </div>
