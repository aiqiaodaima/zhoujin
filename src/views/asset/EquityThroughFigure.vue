  ...
  export default {
    ...
    watch:{
      jsonData:{
        handler(jsonData) {
          if (jsonData.data){
            console.log('result:', this.jsonData.data)
      ...
      graphTree(config) {
        ...
        function update(source, originalData, g) {
          ...
          nodeEnter.append('text')
            ...
            .text((d) => {
              // 大于16  //暂时不显示出资金额
              const str = (d.name === 'origin') ? '' : d.a ? '出资金额:' + d.a+'（'+d.curCode+'）' : ''
