...
<template>
  <a-form-model ref="counterPartyForm" :model="form" :rules="rules">
    <a-card  style="margin-top: 12px" :bordered="false" title="受让方子账户">
      <a-row>
        <BiCol :md='{span:12,offset:1}' :lg='{span:12,offset:1}' :xl='{span:12,offset:1}'>
          <BiFormItemSelect :disabled="disabled" :options="cusAccInfoList" customKey="accNo" customVal="accName"  label="受让方子账户" name="accNo"  v-model="form.accNo" @change="changeAccNo"  showSearch/>
        </BiCol>
        <BiCol :md='{span:12,offset:1}' :lg='{span:12,offset:1}' :xl='{span:12,offset:1}'>
          <BiFormItemSelect :disabled="disabled" :options="bnfs" customKey="proCode" customVal="proName"  label="受益人" name="proCode"  v-model="form.proCode" @change="changeProCode"  showSearch/>
        </BiCol>
      </a-row>
      <a-table
        ref="table"
