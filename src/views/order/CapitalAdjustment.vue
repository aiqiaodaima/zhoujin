  ...
  export default {
    ...
    methods: {
      ...
      async changePrjCode(target){
        ...
        this.counterParty.rcvBankCode = target.bankCode
        ...
        this.counterParty.totalAmtDate = (target.savingBal===null?'--':target.savingBal) + '  (' + (target.updateDate===null?'--':target.updateDate) + ')'
      ...
      submit(button){
        ...
        let param = {
          ...
          trxOrders:  {...counterParty,trantypeCode:'LOA1401',trustPrjName:this.counterParty.trustPrjName,status:this.status,batchId:this.batchId},
      ...
      async doSaveAdjust(data,button) {
        ...
        // 上传文件
        this.$refs.orderOther.uploadFiles('LOA1401',this.batchId)
      },
      async getAdjustInfo(processId){
        ...
        if(trxOrders.status === '80' || trxOrders.status === '5'){
          this.payDisabled.disabled = false
        }else{
