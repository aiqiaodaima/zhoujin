  ...
  <j-modal
    ...
    <a-spin :spinning="confirmLoading" style="margin: 5px;">
      <a-form :form="form" ref="form" formLayout="horizontal">
        ...
        <a-card size="small" title="股权信息" :bordered="true">
          <a-row :gutter="24">
          ...
          <a-row :gutter="24">
            <BiCol><BiFormItemInput :disabled="disabled" label="公司名称" :decorator="['cusAssetEquity.comName', rules.globalReq]"/></BiCol>
            <BiCol><BiFormItemInput :disabled="disabled" label="机构代码" :decorator="['cusAssetEquity.orgCode', rules.globalReq]"/></BiCol>
            <BiCol><BiFormItemSelect :disabled="disabled" label="币种" :options="dicts.CUR_CODE" :decorator="['cusAssetEquity.curCode', rules.globalReq]"/></BiCol>
            <BiCol><BiFormItemNumPercent :disabled="disabled" label="受托股权价值" :decorator="['cusAssetEquity.trusetEquVal', rules.globalReq]"/></BiCol>
            <BiCol><BiFormItemNumPercent :disabled="disabled" label="交付日股权比例" :decorator="['cusAssetEquity.trusetEquRto', rules.globalReq]"/></BiCol>
            <BiCol><BiFormItemSelect :disabled="disabled" label="财产管理机构" :decorator="['cusAssetEquity.amOrgCode', {}]" :options="orgList" custom-key="orgCode" custom-val="orgName"/></BiCol>
            ...
            <BiCol><BiFormItemTextarea :disabled="disabled" label="备注" :decorator="['cusAssetEquity.subfundsDivRul', {}]"/></BiCol>
          </a-row>
        </a-card>
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        url: {
          save: "/prj/asset/saveEquity",
          upload: "/sys/common/biUpload",
          find: "/prj/asset/findEquity"
        }
      }
    },
    computed:{
      ...
      handleOk(){
        ...
        this.form.validateFields((err, values) => {
          ...
          if (!err) {
            ...
            formData.cusAssetEquity.prjCode = that.param.prjCode;
            formData.cusAssetEquity.periodNo = that.param.periodNo;
            formData.cta.trustPropertyAmt = formData.cusAssetEquity.trusetEquVal;
            postAction(that.url.save, formData).then((res)=>{
              if(res.success){
                that.addOrdersOptLogs({
                  ...
                  oprations: '委托资产保存',
                  status: '待提交',
                  memo:formData.cusAssetEquity.comName
                });
                ...
                if(filesNeed2Upload.length>0){
                  ...
                  formData.append("module","ASSET_EQUITY");
